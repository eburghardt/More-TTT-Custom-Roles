...
local function RoleChatMsg(sender, role, msg)
	...
	net.WriteString(msg)
	if role == ROLE_TRAITOR or role == ROLE_ZOMBIE or role == ROLE_HYPNOTIST or role == ROLE_VAMPIRE r role == ROLE_CURSED or role == ROLE_ASSASSIN then
...
function GetInnocentFilter(alive_only)
	return GetPlayerFilter(function(p) return (not (p:IsTraitor() or p:IsZombie() or p:IsHypnotist() or p:IsVampire() or p:IsCursed() or p:IsAssassin())) and (not alive_only or p:IsTerror()) end)
end

function GetTraitorsFilter(alive_only)
	return GetPlayerFilter(function(p) return (p:IsTraitor() or p:IsZombie() or p:IsHypnotist() or p:IsVampire() or p:IsCursed() or p:IsAssassin()) and (not alive_only or p:IsTerror()) end)
end

function GetTraitorsAndJestersFilter(alive_only)
	return GetPlayerFilter(function(p) return (p:IsTraitor() or p:IsZombie() or p:IsHypnotist()  or p:IsCursed() or p:IsAssassin() or p:IsJester() or p:IsSwapper()) and (not alive_only or p:IsTerror()) end)
end
...
function GM:PlayerSay(ply, text, team_only)
	...
	if GetRoundState() == ROUND_ACTIVE then
		...
		elseif team_only and not team and (ply:IsTraitor() or ply:IsZombie() or ply:IsHypnotist() or ply:IsVampire() or ply:IsCursed() or ply:IsAssassin() or ply:IsDetective() or ply:IsJester() or ply:IsSwapper()) then

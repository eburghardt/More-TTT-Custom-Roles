	...
	if DetectiveMode() then
		if p:IsSpec() and (not p:Alive()) then
			...
			elseif p:GetNWBool("body_found", false) then
				...
				if client:IsSpec() or
						client:IsActiveTraitor() or client:IsActiveHypnotist() or client:IsActiveVampire() or client:IsActiveAssassin() or client:IsActiveCursed() or client:IsActiveZombie() or

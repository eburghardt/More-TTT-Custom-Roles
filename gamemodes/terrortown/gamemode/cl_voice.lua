...
local function RoleChatRecv()
	...
	elseif role == ROLE_VAMPIRE then
		...
		elseif role == ROLE_CURSED then
			chat.AddText(Color(45, 45, 45),
				Format("(%s) ", string.upper(GetTranslation("cursed"))),
				Color(100, 100, 100),
				sender:Nick(),
				Color(255, 255, 255),
				": " .. text)
...
function GM:OnPlayerChat(ply, text, teamchat, dead)
	...
	if teamchat and (not team and not (ply:IsTraitor() or ply:IsZombie() or ply:IsHypnotist() or ply:IsVampire() or ply:IsCursed() or ply:IsAssassin() or ply:IsDetective() or ply:IsDetraitor()) or team) then
		teamchat = false
	end
...
function GM:PlayerStartVoice(ply)
	...
	if client == ply then
		if client:IsActiveTraitor() or client:IsActiveHypnotist() or client:IsActiveVampire() or client:IsActiveCursed() or client:IsActiveAssassin() or client:IsActiveZombie() then
	...
	if client:IsActiveTraitor() or client:IsActiveHypnotist() or client:IsActiveVampire()  or client:IsActiveCursed()or client:IsActiveAssassin() or client:IsActiveZombie() then
		...
		elseif ply:IsActiveTraitor() or ply:IsActiveHypnotist() or ply:IsActiveVampire() or ply:IsActiveCursed() or ply:IsActiveAssassin() or ply:IsActiveZombie() then
	...
	-- run ear gesture
	if not ((ply:IsActiveTraitor() or ply:IsActiveHypnotist() or ply:IsActiveVampire() or ply:IsActiveCursed() or ply:IsActiveAssassin() or ply:IsActiveZombie()) and (not ply.traitor_gvoice)) then
...
local function ReceiveVoiceState()
	...
	if (not IsValid(LocalPlayer())) or (not (LocalPlayer():IsActiveTraitor() or LocalPlayer():IsActiveHypnotist() or LocalPlayer():IsActiveVampire() or LocalPlayer():IsActiveCursed() or LocalPlayer():IsActiveAssassin() or LocalPlayer():IsActiveZombie())) then return end
...
local function IsTraitorChatting(client)
	return (client:IsActiveTraitor() or client:IsActiveHypnotist() or client:IsActiveVampire() or client:IsActiveCursed() or client:IsActiveAssassin() or client:IsActiveZombie()) and (not client.traitor_gvoice)
end

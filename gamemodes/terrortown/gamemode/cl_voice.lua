...
function GM:OnPlayerChat(ply, text, teamchat, dead)
	...
	if teamchat and (not team and not (ply:IsTraitor() or ply:IsZombie() or ply:IsHypnotist() or ply:IsVampire() or ply:IsCursed() or ply:IsAssassin() or ply:IsDetective()) or team) then
		teamchat = false
	end
